 ...
 */
public class AnoLancamentoObserver extends AbstractObserver {
  public AnoLancamentoObserver(Predicate<Disco> filter, Notifier notifier) {
  ...
  public void update(Disco disco) {
    if (super.getFilter().test(disco)) {
      super.getNotifier().notify("Novo disco lançado em " + disco.getAnoLancamento() + " adicionado!");
    }
  }
}
