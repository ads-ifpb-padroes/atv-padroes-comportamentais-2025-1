 ...
 */
public class GeneroObserver extends AbstractObserver {
  public GeneroObserver(Predicate<Disco> filter, Notifier notifier) {
  ...
  public void update(Disco disco) {
    if (super.getFilter().test(disco)) {
      super.getNotifier().notify("Novo disco do genero " + disco.getGenero() + " adicionado!");
    }
  }
}
