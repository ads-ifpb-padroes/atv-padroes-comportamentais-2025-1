...
public class InteresseGenero extends InteresseBase {

    public InteresseGenero(String valor, List<ServicoNotificacao> servicos) {
        super(valor, servicos);
    }

    @Override
    public boolean corresponde(Disco disco) {
        return disco.getGenero().toLowerCase().contains(valor.toLowerCase());
    }

    @Override
    protected String gerarMensagem(Disco disco) {
        return String.format("Novo disco do genêro " + valor + "adicionado\n", disco.getArtista());
    }
